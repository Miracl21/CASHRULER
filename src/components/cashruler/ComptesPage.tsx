    ...
    <div className="p-4 flex flex-col h-full overflow-y-auto overflow-x-hidden">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => handleOpenCompteForm()} className="shadow-sm">
          <PlusCircle className="mr-2 h-4 w-4" /> Nouveau Compte
        </Button>
      </div>
      ...
      <div className="flex-grow">
        {comptes.length === 0 ? (
          ...
          comptes.map(compte => {
            ...
            return (
              <Card key={compte.id} className={cn("mb-6 shadow-md border-l-4")} style={{ borderColor: compte.color || 'hsl(var(--primary))' }}>
                <CardHeader className="pb-3 pt-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-xl flex items-center" style={{ color: compte.color || 'hsl(var(--foreground))' }}>
                        <CompteIcon className={cn("mr-2 h-6 w-6")} />
                        {compte.name}
                        <LockStatusIcon className={cn("ml-2 h-4 w-4", lockColorClass)} title={lockTooltip} />
                      </CardTitle>
                      <CardDescription className="text-xs mt-1">
                        {typeDetails?.label || compte.type}
                        {compte.targetAmount && compte.targetAmount > 0 ? ` | Objectif: ${compte.targetAmount.toLocaleString('fr-FR')} ${CURRENCY_SYMBOL}` : ''}
                        {compte.isPredefined && <span className="ml-1 text-xs font-semibold text-muted-foreground">(Prédéfini)</span>}
                      </CardDescription>
                    </div>
                ...
                <CardContent className="space-y-3">
                  <div className="text-2xl font-bold" style={{ color: compte.color || 'hsl(var(--foreground))' }}>
                    {balance.toLocaleString('fr-FR')} {CURRENCY_SYMBOL}
                  </div>
                  ...
                  <div className="flex space-x-2">
                    <Button
                      ...
                      style={{ color: compte.color || 'hsl(var(--primary))', borderColor: compte.color ? `${compte.color}80` : 'hsl(var(--primary)/0.5)' }}
                      disabled={compte.id !== PREDEFINED_COMPTE_COURANT_ID && compte.lockStatus === 'full'}
                    >
                      <DollarSign className="mr-2 h-4 w-4" /> Alimenter Compte
                    </Button>
                  </div>
                    ...
                    <div className="mt-4 pt-3 border-t">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="text-md font-semibold text-foreground flex items-center">
                          <PackageSearch className="mr-2 h-5 w-5 text-secondary" />Objectifs d'Achat
                        </h4>
                        {(compte.lockStatus !== 'full' && compte.lockStatus !== 'outgoing_only') && (
                          <Button size="sm" variant="outline" onClick={() => handleOpenGoalForm(undefined, compte.id)} className="text-xs">
                            <PlusCircle className="mr-1 h-3 w-3" /> Nouvel Objectif
                          </Button>
                        )}
                      </div>
                      {goalsInThisCompte.length === 0 ? (
                        <p className="text-xs text-muted-foreground text-center py-2">Aucun objectif d'achat pour ce compte.</p>
                      ) : (
